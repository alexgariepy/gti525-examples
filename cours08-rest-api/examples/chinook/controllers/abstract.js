export class Response {
  ...
  static created(results) {
    return new Response(201, "Created", results);
  }
...
export class AbstractController {
  ...
  createLinks(req, count, page, pageSize) {
    ...
    if (page > 1) {
      ...
      links.previous = `${req.baseUrl}${req.path}?${new URLSearchParams(
        query
      ).toString()}`;
    }
    ...
    if (count >= page * pageSize) {
      query.page = page + 1;
      links.next = `${req.baseUrl}${req.path}?${new URLSearchParams(
        query
      ).toString()}`;
    }
    return links;
